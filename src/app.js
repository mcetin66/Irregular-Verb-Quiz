...
const GROUPS_SPM = [
  ...
  {
    name: "Rotor ve mıknatıs", tab: "geo",
    items: [
...
const GROUPS_SCIM = [
  ...
  {
    name: "Eşdeğer devre (faz başına)", tab: "perf",
    items: [
  ...
  {
    name: "Isıl ve mekanik", tab: "therm",
    items: [
...
const CHOICES = [
  { tab: "mat", key: "lamGrade", label: "Silisli sac", opts: () => Object.keys(LAMINATIONS) },
  { tab: "mat", key: "cageMat", label: "Rotor kafesi", opts: () => Object.keys(CAGE_MATERIALS) },
  { tab: "therm", key: "insulClass", label: "Yalıtım sınıfı", opts: () => Object.keys(INSULATION) },
  { tab: "therm", key: "cutMethod", label: "Sac kesim yöntemi", opts: () => Object.keys(CUT_METHODS) },
  { tab: "therm", key: "impreg", label: "Emprenye", opts: () => Object.keys(IMPREG) },
  { tab: "perf", key: "RrMode", label: "Rotor direnci kaynağı",
    opts: () => ["manual", "geometri"],
    text: { manual: "Doküman değeri", geometri: "Geometriden hesapla" } },
];
const TOGGLES = [
  ["rotorClosed", "Kapalı rotor oluğu", "geo"],
  ["skinEffect", "Derin çubuk (deri) etkisi", "perf"],
  ["anneal", "Gerilim giderme tavı", "therm"],
  ["hallRing", "Hall mıknatıs halkası (mil üzeri)", "therm"],
];

/* Sekmeler: profesyonel araçlardaki gibi iş akışı aşamasına göre.
   Her sekme kendi GİRDİLERİNİ ve o girdilerin doğrudan SONUCUNU taşır. */
const TABS = [
  ["geo",   "Geometri"],
  ["wind",  "Sargı"],
  ["mat",   "Malzeme"],
  ["perf",  "Performans"],
  ["therm", "Isıl ve üretim"],
  ["check", "Denetim"],
];
...
<div class="stream">
<nav class="tabs" id="tabs" role="tablist" aria-label="Tasarım aşaması"></nav>

<!-- GEOMETRİ -->
<section data-tab="geo"><h2>Geometri ayarları</h2><div class="card" id="ctrls-geo"></div></section>
<section data-tab="geo" id="sec-slots" hidden><h2>Oluk kombinasyonu</h2><div class="card">
  <div id="slotrules"></div>
  <p class="note" id="slotsugg"></p>
</div></section>
<section data-tab="geo"><h2>Oluk / kutup matrisi</h2><div class="card">
...
<!-- SARGI -->
<section data-tab="wind"><h2>Sargı ayarları</h2><div class="card" id="ctrls-wind"></div></section>
<section data-tab="wind"><h2>Sargı yerleşimi</h2><div class="card">
  <div class="scroller"><div class="layout" id="layout"></div></div>
  <p class="note" id="layout-note"></p>
</div></section>
<section data-tab="wind"><h2>Sargı faktörü harmonikleri</h2><div class="card"><div class="harm" id="harm"></div></div></section>

<!-- MALZEME -->
<section data-tab="mat"><h2>Malzeme seçimi</h2><div class="card" id="ctrls-mat"></div></section>
<section data-tab="mat" id="sec-mat" hidden><h2>Silisli sac</h2><div class="card">
...
<!-- PERFORMANS -->
<section data-tab="perf"><h2>Besleme ve çalışma noktası</h2><div class="card" id="ctrls-perf"></div></section>
<section data-tab="perf"><h2>Sonuçlar</h2><div class="card"><div class="results" id="results"></div></div></section>
<section data-tab="perf" id="sec-curve" hidden><h2>Moment / devir</h2><div class="card">
  <div id="curve"></div>
  <p class="note">Yatay eksen devir (d/dk), dikey eksen moment (N·m).</p>
</div></section>
<section data-tab="perf" id="sec-plate" hidden><h2>Etiket karşılaştırması</h2><div class="card">
  <div class="rows" id="plate"></div>
  <p class="note">Sol sütun modelin hesabı, sağ sütun dokümandaki değer.</p>
</div></section>

<!-- ISIL VE ÜRETİM -->
<section data-tab="therm"><h2>Isıl, mekanik ve üretim ayarları</h2><div class="card" id="ctrls-therm"></div></section>
<section data-tab="therm" id="sec-accept" hidden><h2>Kabul testleri ve ısıl denetim</h2><div class="card">
...
<section data-tab="therm" id="sec-env" hidden><h2>Besleme zarfı ve üretim etkileri</h2><div class="card">
...
<!-- DENETİM -->
<section data-tab="check"><h2>Tasarım kontrolü</h2><div class="card" id="checks"></div></section>
<section data-tab="check"><h2>Dışa aktar</h2><div class="card">
  <div class="actions">
 ...
 * Sekmeler
 *
 * Profesyonel motor tasarım araçları (Motor-CAD, RMxprt) girdileri
 * konu başlığına göre değil, İŞ AKIŞI aşamasına göre ayırır ve her
 * aşamanın girdisiyle o girdinin doğrudan sonucunu yan yana tutar.
 * Kesit görünümü ise aşamadan bağımsız, her zaman ekranda kalır.
 * ------------------------------------------------------------------ */
let tab = localStorage.getItem("motor.tab") || "geo";
...
function applyTab() {
  ...
  for (const sec of document.querySelectorAll("section[data-tab]"))
    sec.classList.toggle("off", sec.dataset.tab !== tab);
  // Etkin sekmeyi yalnızca ÇUBUĞUN içinde ortala; scrollIntoView sayfanın
  // kendisini de yatay kaydırdığı için elle hesaplanır.
  const cur = tabBar.querySelector('[aria-selected="true"]');
  if (cur) tabBar.scrollLeft = cur.offsetLeft - (tabBar.clientWidth - cur.offsetWidth) / 2;
...
function buildControls() {
  const hosts = Object.fromEntries(
    TABS.map(([k]) => [k, document.getElementById(`ctrls-${k}`)]).filter(([, h]) => h));
  for (const h of Object.values(hosts)) h.replaceChildren();
  const host = hosts.geo;             // sekmesi belirtilmemiş grupların yeri
  const into = (t) => hosts[t] ?? host;
  ctrlEls = {};
  ...
  if (D.type !== "scim") {
    for (const [k, h] of Object.entries(hosts))
      if (!h.children.length) h.appendChild(el("p", "note", "Bu makine tipinde bu aşamada ayar yok."));
    return;
  }
  ...
  for (const ch of CHOICES) {
    ...
    gEl.appendChild(box);
    into(ch.tab).appendChild(gEl);
  }

  // Anahtarlar ait oldukları aşamaya dağıtılır
  for (const t of new Set(TOGGLES.map((x) => x[2]))) {
    const list = TOGGLES.filter((x) => x[2] === t);
    const tEl = el("div", "group");
    tEl.appendChild(el("span", "lbl", "Seçenekler"));
    ...
    into(t).appendChild(tEl);
  }

  into("mat").appendChild(el("p", "note", CAGE_MATERIALS[D.cageMat]?.note ?? ""));
  into("therm").appendChild(
